...
export const TRAINING_SCENARIOS: TrainingScenario[] = [
  // ── WM ──
  {
    ...
    workflow_diagram: [
      "시작",
      "Send Message (인사말)",
      "Answer Generation (상품/수수료 문서)",
      "Answer in Q&A (FAQ 매칭)",
      "Add Condition (확신도 체크)",
      "Assign Member Connection (담당자 연결)",
    ],
    workflow_steps: [
      {
        step: 1,
      ...
      },
      {
        ...
        detail:
          "금융상품 설명서(ELS, DLS, 펀드), 수수료 안내서를 폴더별로 분류하여 업로드합니다. 각 문서에 해시태그를 부여하고, 폴더별 접근 권한을 설정합니다.",
        docRef:
      ...
      },
      {
        ...
        detail:
          '앱 이름/설명 입력 후, 시작 노드에서 Send Message 노드를 추가합니다. 메시지: "안녕하세요, 하나증권 AI 상담입니다. 무엇을 도와드릴까요?"',
        nodes: ["Send Message"],
        docRef:
      ...
      {
        step: 4,
        title: "Answer Generation 노드 추가 (문서 기반 답변)",
        where: "플로우 빌더 > Send Message 다음에 노드 추가",
        action:
          "Answer Generation 노드를 추가하고 검색 소스로 문서 + Q&A를 설정",
        detail:
          '검색 소스: "문서"(고객지원 폴더 지정) + "Q&A"(등록된 FAQ) 모두 활성화. 실행 유형은 "Agent" 모드로 설정하여 멀티턴 대화가 가능하도록 합니다. 답변을 변수(@ANSWER)에 저장합니다.',
      ...
      },
      {
        ...
        where: "플로우 빌더 > Answer Generation 다음에 노드 추가",
        action:
          "Add Condition 노드 + Assign Member Connection 노드로 에스컬레이션 구성",
        detail:
          'Add Condition 노드에서 @ANSWER 변수가 "문서에서 관련 정보를 찾지 못했습니다" 등을 포함하면 Assign Member Connection 노드로 분기합니다. 해당 노드에서 담당자/팀을 지정하고 실시간 채팅 또는 이메일 전달을 설정합니다.',
        nodes: ["Add Condition", "Assign Member Connection"],
        docRef:
  ...
  // ── IB ──
  {
    ...
    alli_app_type: "interactive",
    alli_nodes: ["Send Message", "Deep Research"],
    difficulty: "intermediate",
    duration_minutes: 60,
    ice_impact: 9,
    ice_confidence: 8,
    ...
    workflow_diagram: [
      "시작",
      "Send Message (리서치 주제 안내)",
      "Deep Research (문서+인터넷 소스) [터미널]",
    ],
    workflow_steps: [
      {
        step: 1,
      ...
      {
        step: 2,
        title: "대화형 앱 생성 및 Send Message 노드",
        where: "Alli 대시보드 > 앱 관리 > 대화형 앱 생성",
        action:
          '"딜 리서치 분석기" 앱 생성 후 Send Message 노드 추가',
        detail:
          'Send Message에서 "분석할 산업/주제를 입력하세요"라는 안내와 함께 Ask Question 옵션(텍스트 입력)을 활성화하여 사용자 입력을 변수에 저장합니다.',
        nodes: ["Send Message"],
        docRef:
          "https://guide.allganize.ai/alli/%EB%A9%94%EC%8B%9C%EC%A7%80-%EB%B3%B4%EB%82%B4%EA%B8%B0-%EC%A7%88%EB%AC%B8%ED%95%98%EA%B8%B0-%EB%85%B8%EB%93%9C/?lang=ko",
      },
      {
        step: 3,
        title: "Deep Research 노드 추가 및 소스 설정",
        where: "플로우 빌더 > Send Message 다음에 Deep Research 노드",
        action:
          "Deep Research 노드를 터미널(마지막) 노드로 추가하고 검색 소스 설정",
        detail:
          '질문 소스: 이전 단계의 사용자 입력 변수. 검색 소스 3가지를 설정합니다: (1) 문서 - 업로드된 리서치 폴더 지정 (2) 인터넷 - 신뢰 URL 지정(금감원 dart.fss.or.kr, KRX krx.co.kr) (3) 업로드 파일 - 사용자가 추가 파일 첨부 가능. 주의: Deep Research는 반드시 마지막 노드여야 합니다.',
        nodes: ["Deep Research"],
        docRef:
    ...
    eval_criteria: [
      ...
      "Deep Research 노드가 터미널 노드로 올바르게 배치",
      "연구 보고서 자동 생성 성공",
      "출처 추적 및 원본 확인",
    ],
    reference_links: [
      {
        label: "Deep Research 노드 가이드",
  ...
  // ── S&T ──
  {
    ...
    alli_app_type: "interactive",
    alli_nodes: ["LLM Input", "LLM Node", "Send Message"],
    difficulty: "beginner",
    duration_minutes: 45,
    ice_impact: 8,
    ice_confidence: 9,
    ...
    workflow_steps: [
      ...
      {
        step: 2,
        title: "대화형 앱 생성 및 LLM Input 노드",
        where: "Alli 대시보드 > 앱 관리 > 대화형 앱 생성",
        action:
          '"리서치 요약기" 앱을 생성하고 LLM Input 노드 추가',
        detail:
          'LLM Input 노드의 소스 타입을 "문서(Document)"로 설정합니다. 범위를 리서치 폴더로 지정하고, 사용자 검색을 활성화하여 원하는 보고서를 선택할 수 있게 합니다. 또는 "업로드(Upload)" 소스로 설정하여 사용자가 직접 파일을 첨부하도록 합니다.',
        nodes: ["LLM Input"],
        docRef:
      ...
      {
        step: 3,
        title: "LLM Node 추가 (요약 프롬프트)",
        where: "플로우 빌더 > LLM Input 다음에 LLM Node",
        action:
          'LLM Node를 "단일 프롬프트" 모드로 설정하고 요약 프롬프트 작성',
        detail:
    ...
    eval_criteria: [
      "LLM Input → LLM Node → Send Message 플로우 완성",
  ...
  // ── 경영지원 ──
  {
    ...
    workflow_diagram: [
      "시작",
      "Send Input Form (거래유형/금액/당사자)",
      "Answer Generation (규정 문서 검색)",
      "LLM Node (위반 가능성 분석)",
      "Add Condition (위반 여부 분기)",
      "Assign Member Connection (준법감시 담당자)",
      "Send Message (참고용 고지)",
    ],
    workflow_steps: [
      {
      ...
      {
        step: 4,
        title: "LLM Node (위반 가능성 분석)",
        where: "플로우 빌더 > Answer Generation 다음",
        action:
          "LLM Node로 검색된 규정 기반 위반 가능성 분석",
        detail:
      ...
      },
      {
        ...
        where: "플로우 빌더 > LLM Node 다음",
        action:
          "위반 가능성이 높으면 준법감시 담당자에게 에스컬레이션",
        detail:
          'Add Condition: @분석결과가 "위반가능성: 높음"을 포함하면 → Assign Member Connection(준법감시 담당자). 그 외 → Send Message("본 분석은 참고용이며, 최종 판단은 준법감시 담당자가 수행합니다").',
        nodes: ["Add Condition", "Assign Member Connection", "Send Message"],
      },
      {
        step: 6,
    ...
    eval_criteria: [
      "6개 노드 플로우 완성 (입력폼→검색→분석→분기→연결→메시지)",
