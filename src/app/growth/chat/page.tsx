...
export default function GrowthChatPage() {
  ...
  // Load member count for header
  useEffect(() => {
    if (!activeCohort) return;
    fetch(`/api/growth/members?cohort_id=${activeCohort.id}`)
      .then((r) => r.json())
      .then((data) => Array.isArray(data) && setMemberCount(data.length));
  }, [activeCohort]);

  // Initial load + Supabase Realtime
  useEffect(() => {
    if (!user || !activeCohort) return;
    fetchMessages();
    ...
    const channel = supabase
      ...
      .on(
        "postgres_changes",
        {
          ...
          filter: `cohort_id=eq.${activeCohort.id}`,
        },
        (payload) => {
          const msg = payload.new as ChatMessage;
          setMessages((prev) => {
            // Deduplicate: remove temp optimistic and add real
    ...
    <div className="h-full flex flex-col bg-gray-50">
      ...
      <div className="px-4 sm:px-6 py-3 border-b border-gray-200 bg-white shrink-0">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-sm font-bold text-gray-900">{activeCohort.name} · 팀 채팅</h2>
            <p className="text-[11px] text-gray-400 mt-0.5 flex items-center gap-1">
              <span className="w-1.5 h-1.5 rounded-full bg-green-400 inline-block" />
              {user.displayName}
              {memberCount != null && (
                <span className="ml-1 flex items-center gap-0.5">
                  <Users size={10} /> {memberCount}명
                </span>
              )}
            </p>
          </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-4 space-y-0.5">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-gray-400 gap-2">
            ...
            <p className="text-xs">{activeCohort.name} 팀 채팅방</p>
          </div>
        ) : (
          messages.map((msg) => {
              ...
              <div key={msg.id}>
                {showDate && (
                  <div className="flex items-center justify-center my-4">
                    <span className="text-[10px] bg-gray-200/80 text-gray-500 px-3 py-0.5 rounded-full">
                      {dateStr}
                    </span>
                  </div>
                )}
                <div className={`flex ${isMe ? "justify-end" : "justify-start"} mb-1 items-end gap-1.5`}>
                  ...
                  <div className={`max-w-[72%] ${isMe ? "items-end" : "items-start"} flex flex-col`}>
                    ...
                    {!isMe && (
                      <span className="text-[10px] font-semibold text-gray-500 ml-1 mb-0.5">
                        {msg.sender_name}
                      </span>
                    )}
      ...
      <div className="px-4 sm:px-6 py-3 bg-white border-t border-gray-200 shrink-0">
        <form onSubmit={sendMessage} className="flex items-center gap-2">
          <input
            ...
            placeholder={`${activeCohort.name} 팀원들에게 메시지...`}
